    ...
    try:
        ...
        if 'Kupci' in xl.sheet_names:
            ...
            for _, row in df.iterrows():
                ...
                try:
                    q("INSERT INTO kupci (naziv, pib, mb, adresa, mesto, jbkjs) VALUES (?,?,?,?,?,?)",
                      (naziv, pib, mb, str(row.get('ADRESA','')), str(row.get('MESTO','')), jbkjs), commit=True)
                    count += 1
                except: pass
            results.append(f"Kupci: {count} uvezeno")
        # Cenovnik
        if 'Cenovnik' in xl.sheet_names:
            ...
            for _, row in df.iterrows():
                ...
                try:
                    ...
                    q("INSERT INTO cenovnik (naziv, cena, pdv, status) VALUES (?,?,?,?)",
                      (naziv, cena, pdv, status), commit=True)
                    count += 1
                except: pass
        ...
        if 'Opis' in xl.sheet_names:
            ...
            count = 0
            for _, row in df.iterrows():
                naziv = str(row.iloc[1]).strip() if len(row) > 1 else ''
                if not naziv or naziv == 'nan': continue
                datum = str(row.iloc[2]).strip() if len(row) > 2 else ''
                if datum == 'nan': datum = ''
                try:
                    q("INSERT INTO dogadjaji (naziv, datum, status) VALUES (?,?,?)",
                      (naziv, datum, 'AKTIVNA'), commit=True)
                    count += 1
                except: pass
        ...
        if 'Evidencija' in xl.sheet_names:
            ...
            for _, row in df.iterrows():
                ...
                try:
                    stavke_json = json.dumps([{"naziv": sv('STAVKE'), "kolicina": 1, "cena": fv('OSNOV'), "pdv": 20}], ensure_ascii=False)
                    q("INSERT INTO evidencija (broj, datum_izd, datum_val, kupac_naziv, kupac_pib, kupac_jbkjs, dogadjaj_naziv, osnov, pdv, ukupno, status, napomena, stavke, korisnik_ime) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                      (broj, sv('DATUM_IZD'), sv('DATUM_VAL'), sv('KUPAC_NAZIV'),
                       sv('KUPAC_PIB').replace('.0',''), sv('KUPAC_JBKJS').split('.')[0],
                       sv('DOGADJAJ_NAZIV'), fv('OSNOV'), fv('PDV'), fv('UKUPNO'),
                       sv('STATUS','ČEKA'), sv('NAPOMENA'), stavke_json, 'Import'), commit=True)
                    count += 1
                except: pass
